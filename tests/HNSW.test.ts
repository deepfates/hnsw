...
describe('HNSW', () => {
  ...
  it('uses efSearch for query-time exploration with overrides', async () => {
    ...
    expect(spy).toHaveBeenNthCalledWith(1, [3, 4, 5, 6, 7], expect.anything(), 0, 4, expect.anything());

    hnsw.searchKNN([3, 4, 5, 6, 7], 2, { efSearch: 3 });
    expect(spy).toHaveBeenNthCalledWith(2, [3, 4, 5, 6, 7], expect.anything(), 0, 3, expect.anything());
    spy.mockRestore();
  });
