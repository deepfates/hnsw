...
export class HNSW {
    ...
    for (let level = this.levelMax; level >= 0; level--) {
      while (true) {
        ...
        for (const neighborId of currentNode.neighbors[level]) {
          ...
          const similarity = cosineSimilarity(node.vector, neighborNode.vector);
        ...
        if (nextNode && maxSimilarity > cosineSimilarity(node.vector, closestNode.vector)) {
  ...
  searchKNN(query: Float32Array | number[], k: number): { id: number; score: number }[] {
    ...
    const candidates = new PriorityQueue<number>((a, b) => {
      ...
      return cosineSimilarity(query, bNode.vector) - cosineSimilarity(query, aNode.vector);
    });
